...
const NavBar = () => {
    ...
    return(<nav className={`liby-nav navbar navbar-expand-sm justify-content-center text-center d-flex ${isMobNavOpen ? `` : `toggle-mob-nav-height`}`}>
        <ul className="navbar-nav">
            ...
            <NavLink className={`nav-item pe-sm-4 ${isMobNavOpen ? `show-item` : `hide-item`} des-item`} to="/" onClick={()=>setIsMobNavOpen(!isMobNavOpen)} active="true">Home</NavLink>
            ...
            { role === "User" ? <NavLink className={`nav-item ${isMobNavOpen ? `show-item` : `hide-item`} des-item`} to="/profile" onClick={()=>setIsMobNavOpen(!isMobNavOpen)}>Profile</NavLink>
            : role === "Librarian" ? 
                <NavLink className={`nav-item ${isMobNavOpen ? `show-item` : `hide-item`} des-item`} to="/manage" onClick={()=>setIsMobNavOpen(!isMobNavOpen)}>Manage</NavLink>
            : null }
            ...
            : role === "Librarian" ? 
            <NavLink className={`profile-buffer nav-item`} to="/" disabled>
                <span className="hide-item des-item">
